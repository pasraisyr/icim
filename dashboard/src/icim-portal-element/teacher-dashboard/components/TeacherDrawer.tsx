...
export default function TeacherDrawer({ window }: Props) {
  ...
  // header content
  const drawerContent = useMemo(() => <TeacherDrawerContent />, []);
  const drawerHeader = useMemo(() => <DrawerHeader open={drawerOpen} />, [drawerOpen]);

  return (
    <Box component="nav" sx={{ flexShrink: { md: 0 }, zIndex: 1200 }} aria-label="teacher navigation">
      {!downLG ? (
