...
export async function fetchStudents(): Promise<Student[]> {
  const res = await fetch(`${BASE_URL}/api/Academic/self-registrations/`);
  if (!res.ok) throw new Error('Failed to fetch students');
  return res.json();
}

export async function createStudent(payload: StudentPayload): Promise<Student> {
  const res = await fetch(`${BASE_URL}/api/Frontend/self-registration/`, {
    method: 'POST',
...
export async function updateStudent(id: number, payload: StudentPayload): Promise<Student> {
  const res = await fetch(`${BASE_URL}/api/Frontend/self-registration/${id}/`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  });
...
export const handleVerify = async (id: number) => {
  try {
    await axios.patch(`${BASE_URL}/api/Frontend/self-registration/${id}/`, { status: 'active' });
